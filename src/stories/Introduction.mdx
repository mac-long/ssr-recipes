...
- Typescript once understood and setup properly does make life a lot easier further down the line.
  ...
  - Encourages you to think about all the possible rest that could possible exist for a given component.
...
- Rome for Linting and Formatting... This one was a surprise, minimal configuration due to sane defaults, it's also super quick because it's built with rust! This tool runs lint checks on average at `1.236s` and formatting at a ridiculously quick `0.5s`. You can run these tests with the tiny shell script below.
  - One negative aspect of this is the fact that it's a rather new tool, so certain luxuries I've gotten used to with ESLint and Prettier such as automatic Tailwind Class sorting are duly missed. However the development team have mentions these are likely additions for the future.
- Utilising HeroIcons and HeroPatterns is very useful.
